...
## SECTION 9: SkillSpector Third-Party Skill Gate
### (Automatic security scan before any new skill lands — enforced by hooks, not this file)

Every new or changed SKILL.md — from `~/.claude/skills/`, any installed plugin, or a project's `.claude/skills/` — is scanned by [NVIDIA/SkillSpector](https://github.com/NVIDIA/skillspector) before it's trusted. As with Section 8, CLAUDE.md text cannot make this happen on its own; two `~/.claude/settings.json` hooks do the enforcement, installed at `~/.claude/tools/skillspector/`:

- **`PreToolUse` (matcher `Write`) → `pretooluse_check.py`** — true preventative block. When Claude itself writes a brand-new SKILL.md in-session, its proposed content is scanned (LLM-assisted, via the local `claude` CLI session — no separate API key) before the file lands on disk. `DO_NOT_INSTALL` **or** a scan that fails to complete after retries → the write is denied (fails closed, per your explicit choice to enforce blocking over convenience). Only covers Write of a full SKILL.md — see the Bash gate below for the far more common install path.
- **`SessionStart` → `sweep.py`** — coverage for everything the PreToolUse hook can't see: `/plugin install`, marketplace updates. Diffs a fingerprint manifest (`~/.claude/tools/skillspector/manifest.json`) and LLM-scans only what's new or changed since last time. Skills already installed before this gate existed were grandfathered in unscanned on first run.
- **`PostToolUse` (matcher `Bash`) → `post_bash_check.py`** — `git clone`/`cp`/`unzip` straight into a skills directory (the actual normal way to "pull in a skill off GitHub") never touches the `Write` tool, so the PreToolUse hook above cannot see or block it. This hook fires after every Bash call, does a cheap keyword pre-check (skip unless the command mentions "skill"/"plugin"/".claude"), and if a new/changed skill turns up, scans it immediately and **auto-quarantines** (`mv dir dir.quarantined`) on `DO_NOT_INSTALL` or scan failure — closing the exposure window within the same session instead of waiting for the next `SessionStart`. Quarantined dirs are permanently excluded from rediscovery; restore with `mv dir.quarantined dir` after manual review.

**Known limitations (found during setup, not theoretical):**
- Static-only analysis reliably misses attacks written as prose/example-code inside a non-executable `.md` file — its own false-positive filter treats fenced code blocks in markdown as illustrative. All three hooks therefore run LLM-assisted analysis, not `--no-llm`, despite the added latency (roughly 10–100s per new skill; occasionally longer on retry).
- The `claude_cli` analysis provider has a non-trivial flake rate extracting a clean JSON verdict, especially when invoked recursively from inside an already-running Claude Code session (observed firsthand). A failed scan blocks/quarantines rather than silently allowing it — if a legitimate skill gets stuck, run `skillspector baseline <dir> -o <dir>/.skillspector-baseline.yaml` to accept it manually, or `skillspector scan <dir> --format markdown` to review first.
- Even a clean LLM-assisted verdict is evidence, not proof — treat `DO_NOT_INSTALL` as a strong signal to review, not an infallible one.
- `SessionStart` fires far more often than "a new terminal session" in this harness (observed up to ~19 concurrent `sweep.py` processes piling up from repeated firing within minutes). `sweep.py` now takes an exclusive non-blocking lock (`~/.claude/tools/skillspector/.sweep.lock`) with a 10-minute cooldown, so repeated firing is a near-instant no-op instead of a process pileup — this is a workaround for the observed firing behavior, not a fix to its root cause, which is outside this tool's control.
